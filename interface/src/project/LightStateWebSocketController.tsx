...
function LightStateWebSocketControllerForm(props: LightStateWebSocketControllerFormProps) {
  ...
  const changeLedOn = (event: React.ChangeEvent<HTMLInputElement>) => {
    setData({ led_on: event.target.checked,led_color:data.led_color }, saveData);
  }
  const changeLedColor = (event: React.ChangeEvent<HTMLInputElement>) => {
    setData({ led_on: data.led_on,led_color:Number.parseInt(event.target.value)  }, saveData);
  }
